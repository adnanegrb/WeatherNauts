...
def _build_features(X):
    ...
    # Bloc 1: Valeurs courantes de Paris
    f.append(last)

    # Bloc 2: Moyennes et écarts-types fenêtrés (3h, 6h, 12h, 24h)
    for fi in [F_TEMP, F_WIND, F_RAIN, F_HUM, F_CLOUD]:
        for w in [3, 6, 12, 24]:
            c = paris[:, -w:, fi]
            f += [c.mean(1, keepdims=True), c.std(1, keepdims=True)]

    # Bloc 3: Lags temporels (changement dernière/ancienne heure)
    for fi in [F_TEMP, F_WIND, F_HUM]:
        for lag in [1, 2, 3, 6, 12, 23]:
            f.append((last[:, fi] - paris[:, -(1 + lag), fi]).reshape(-1, 1))

    # Bloc 4: Padding zéros (remplace encodage cyclique supprimé du train)
    f += [
        np.zeros((X.shape[0], 1), dtype=np.float32),
        np.zeros((X.shape[0], 1), dtype=np.float32),
    ]

    # Bloc 5: Voisins géographiques pondérés et anomalies locales
    all_last = X[:, :, -1, :]
    for fi in [F_TEMP, F_WIND, F_RAIN, F_HUM]:
        wm = (all_last[:, :, fi] * W_GEO[None, :]).sum(1, keepdims=True)
        f += [wm, last[:, fi:fi+1] - wm]

    # Bloc 6: Composantes vectorielles du vent (U et V)
    wr = np.deg2rad(paris[:, :, F_WDIR])
    ws = paris[:, :, F_WIND]
    u_p = ws * np.sin(wr)
    v_p = ws * np.cos(wr)
    f += [u_p[:, -1:], v_p[:, -1:], u_p[:, -6:].mean(1, keepdims=True), v_p[:, -6:].mean(1, keepdims=True)]

    # Bloc 7: Advection thermique (vent qui apporte chaud/froid)
    ...
    # Bloc 8: Accélération (dérivée seconde du vent et température)
    ...
    # Bloc 9: Laplacien spatial (diffusion thermique locale)
    for fi in [F_TEMP, F_HUM, F_WIND]:
        lap = ((all_last[:, NEARBY, fi] - all_last[:, PARIS_IDX:PARIS_IDX+1, fi]) / DIST2[None, NEARBY]).mean(1, keepdims=True)
        f.append(lap)

    # Bloc 10: Convergence et cohérence directionnelle du vent
    ...
    # Bloc 11: Pentes linéaires (tendance sur 6h, 12h, 24h)
    for fi in [F_TEMP, F_WIND, F_HUM]:
        for w in [6, 12, 24]:
            c = paris[:, -w:, fi]
    ...
    # Bloc 12: Proxies pluie et résidus thermiques
    hum_last = last[:, F_HUM]
...
class Agent:
    def __init__(self):
        print("[Agent] Chargement des modèles...")
        ...
        print("[Agent] Modèles chargés ✓")

    def predict(self, X_test):
        last = X_test[PARIS_IDX, -1, :]
        feats = _build_features(X_test)

        # Calcul slope_6h: pente thermique sur 6 dernières heures
        ...
        # Prédictions avec correction inertie thermique
